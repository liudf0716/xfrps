...
<script>
  ...
  import { FtpProxy } from '../utils/proxy.js'
  export default {
    data() {
      return {
        proxies: null,
    ...
    methods: {
      ...
      fetchData(cPage) {
        if (cPage == 0) {
          fetch('/api/proxy/ftp', {credentials: 'include'})
          .then(res => {
              return res.json()
            })
          .then(json => {
              ...
              fetch('/api/proxy/ftp/1', {credentials: 'include'})
              .then(res => {
                  return res.json()
                })
              .then(json => {
                  ...
                  for (let proxyStats of json.proxies) {
                    this.proxies.push(new FtpProxy(proxyStats))
                  }
                })
            })
        } else {
          this.parentCurrentpage = cPage;
          fetch('/api/proxy/ftp/'+cPage, {credentials: 'include'})
            .then(res => {
              return res.json()
            }).then(json => {
              this.proxies = new Array()
